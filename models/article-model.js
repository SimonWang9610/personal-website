...
module.exports.getArticles = function(type) {
	let sql = null;
	if (type) {
		sql = 'SELECT a.Guid, a.Subject, a.Category, a.IsPrivated, a.CreationDate, ac.CommentsCount, a.ViewsCount FROM t_article a '
		...
		sql = 'SELECT a.Guid, a.Subject, a.IsPrivated, a.CreationDate, ac.CommentsCount, a.ViewsCount FROM t_article a '
			+ 'LEFT JOIN (SELECT ArticleGuid, COUNT(Guid) AS CommentsCount FROM t_comment GROUP BY ArticleGuid) AS ac '
			+ 'ON ac.ArticleGuid = a.Guid '
			+ 'ORDER BY a.CreationDate DESC';
...
module.exports.getSingleArticle = function(id) {
	let sql = 'SELECT a.Guid, a.Subject, a.Content, a.Category, a.IsPrivated, a.CreationDate, a.LastEditDate, ac.CommentsCount, a.ViewsCount FROM t_article a '
			+ 'LEFT JOIN (SELECT ArticleGuid, COUNT(Guid) AS CommentsCount FROM t_comment GROUP BY ArticleGuid) AS ac '
			+ 'ON ac.ArticleGuid = a.Guid '
			+ 'WHERE a.Guid=?';
	return query.execute({
...
module.exports.getLatestArticle = function() {
	let sql = 'SELECT a.Guid, a.Subject, a.Content, a.CreationDate FROM t_article a '
			+ 'ORDER BY a.CreationDate DESC';
	return query.execute({
		statement: sql
	}).then(rs => {
